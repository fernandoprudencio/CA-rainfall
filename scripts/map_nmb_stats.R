...
# p <- 8
# dat <- getValues(img[[p]])[!is.na(getValues(img[[p]]))]
# length(dat[dat>5])*100/length(dat)
# 
# 
# img[img > 5] <- 5
# img[img < -.5] <- -.5
...
intrv <- c(seq(-.5, 1, .25), 5, 10)
intrv.lbl <- c(seq(-.5, 1, .25), 5, 10)
...
cb.palette <-
  c(
    #' blue scale
    "#053062", "#2167ad", "#4494c5", "#93c7e0", "#ffffff",
    #' red scale
    "#ffffff", "#f6a683", "#d8614e", "#b3182b", "#68001f"
  )
cb.palette <- cptcity::cpt("cmocean_balance")

#'   Define plot name
name <- "export/stats/nmb_stats.png"
#'   Save plot
